let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	if (global.db.data.store.owner.map(([number]) => number).map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(meh[0].jid)) return m.reply('[ ! ] Dia sudah jadi owner.')
	if (meh.length == 0) return m.reply(`[!] Failed, @${(who || '')} bukan pengguna WhatsApp.`, null, { mentions: [meh[0].jid] })
	global.db.data.store.owner.push([who, text[1], true])
