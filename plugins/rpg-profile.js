...
let handler = async (m, { conn, isPrems }) => {
	...
	try {
		...
		str += `*Limit :* ${(global.db.data.prems.some(v=>who2.includes(v)) || global.owner.map(v => v[0]).some(x=>who2.includes(x)) || global.db.data.owner.map(v => v[0]).some(x=>who2.includes(x))) ? '~ Infinity ~' : limit}\n`
