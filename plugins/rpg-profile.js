...
let handler = async (m, { conn, isPrems }) => {
	...
	try {
		let pp, user = await global.db.data.users
		...
		str += `*Limit :* ${(global.db.data.datas.prems.some(v=>who2.includes(v)) || global.db.data.datas.rowner.map(v => v[0]).some(x=>who2.includes(x)) || global.db.data.datas.owner.map(v => v[0]).some(x=>who2.includes(x))) ? '~ Infinity ~' : limit}\n`
