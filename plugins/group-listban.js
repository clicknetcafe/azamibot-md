let handler = async (m, { conn, isOwner }) => {
	...
	let users = Object.entries(global.db.data.users).filter(user => user[1].banned && !(user[0].startsWith('212') || user[0].startsWith('265')))
	let users2 = Object.entries(global.db.data.users).filter(user => user[1].banned)
	let listgc = [], z = 0, txt = ''
	let groups = Object.values(await conn.groupFetchAllParticipating())
	for (let i = 0; i < groups.length; i++) {
		if (global.db.data.chats[groups[i].id].isBanned) {
